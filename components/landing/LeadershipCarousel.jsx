...
export default function LeadershipCarousel() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      const cardWidth = mobile ? vw * 0.2 : vw * 0.18;
  ...
  const DESKTOP_SLOTS = {
    "-1": { x: 0, zIndex: 1 },
    0: { x: metrics.gap * 2, zIndex: 5 },
    1: { x: metrics.gap * 8.58, zIndex: 3 },
    2: { x: metrics.gap * 10.6, zIndex: 2 },
  };

  const MOBILE_SLOTS = {
    0: { x: 0, zIndex: 5 },
    1: { x: metrics.gap * 6.8, zIndex: 2 },
    2: { x: metrics.gap * 9, zIndex: 1 },
    3: { x: metrics.gap * 11.2, zIndex: 1 },
  };
  ...
  /* ---------------- Render ---------------- */
  return (
    <section className="section-block-padding px-2 sm:px-32 xl:px-36 3xl:px-44">
...
<div className="relative pr-5 md:pr-0">
      ...
      >
        ...
        <motion.div
          ...
          style={{
            width: isMobile
              ? metrics.cardWidth * SCALE.inactive.width * 3.55
              : metrics.cardWidth * SCALE.inactive.width * 2,
          }}
        >
          <div className="xl:-ml-6 2xl:-ml-18 sm:ml-2 -ml-2 cursor-pointer">
            <h3 className="fl4">{activeLeader.name}</h3>
            <p className="fl7 mb-4">{activeLeader.position}</p>
            <p className="fl8 text-justify">{activeLeader.description}</p>
          </div>
        </motion.div>
      </div>
</div>
